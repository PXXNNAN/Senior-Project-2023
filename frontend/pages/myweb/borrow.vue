...
<script>
const url = "http://localhost:5000/api/borrow_history1";
export default {
  ...
  methods: {
    async handleBorrowResponse(item, status, status1) {
      ...
      try {
        const res = await this.$axios.post("http://localhost:5000/api/borrow_history/update", {
          newStatus: status,
          borrowId: item.borrow_id,
          updateItem: true
        });
        const res1 = await this.$axios.post(
          "http://localhost:5000/api/borrow_history/update_borrow_status_item",
          {
        ...
        const itemsArr = await this.$axios.get("http://localhost:5000/api/item/2564");
